...
// Where a post says it happened. Never coordinates — a venue or area name only.
//
// The joined place name wins over the stored label because the label is a snapshot
// taken at post time: if a venue is renamed, live rows should follow. The stored label
// is the fallback for posts whose venue has since been deleted (venue_id goes null but
// the post should still say where it was).
export function placeLabel(post) {
  return post?.place?.name ?? post?.placeLabel ?? null;
}
