...
async function placeMap(rows) {
  ...
  // lat/lng ride along so the feed can answer "how far is this from me?" without a
  // round trip — the distance is computed on-device against the viewer's own position.
  const [vRes, eRes] = await Promise.all([
    venueIds.length ? supabase.from('venues').select('id, name, city, category, latitude, longitude').in('id', venueIds) : Promise.resolve({ data: [] }),
    eventIds.length ? supabase.from('events').select('id, title, category, venue_id').in('id', eventIds) : Promise.resolve({ data: [] }),
  ]);
  const map = new Map();
  (vRes.data ?? []).forEach((v) => map.set(`venue-${v.id}`, {
    kind: 'venue', id: v.id, name: v.name, city: v.city ?? null, category: v.category ?? null,
    lat: v.latitude ?? null, lng: v.longitude ?? null,
  }));
  (eRes.data ?? []).forEach((e) => map.set(`event-${e.id}`, {
    kind: 'event', id: e.id, name: e.title, city: null, category: e.category ?? null, lat: null, lng: null,
  }));
  return map;
}
