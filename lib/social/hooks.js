...
import { fetchThread, sendMessage, markThreadRead } from './messagesRepository';
...
export function useSendMessage(meId, otherId) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ body, storyId }) => sendMessage({ senderId: meId, recipientId: otherId, body, storyId }),
