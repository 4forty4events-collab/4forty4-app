...
import { fetchExplorerPassport } from './passportRepository';
...
// The Explorer Passport for any user — also how the viewer learns their own unlock
// state (Explorer Chat / groups / meetups), since the ladder lives in the same RPC.
export function useExplorerPassport(userId) {
  return useQuery({
    queryKey: ['explorerPassport', userId ?? null],
    queryFn: () => fetchExplorerPassport(userId),
    enabled: !!userId,
    staleTime: 60_000,
  });
}
...
// panel is that the number is current.
