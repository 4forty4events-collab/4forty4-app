...
export async function fetchThread(meId, otherId) {
  ...
  return (data ?? []).map((m) => ({
    ...
    postId: m.post_id ?? null,   // the experience this conversation started from
...
// Send a message. `storyId` / `postId` carry the ORIGIN — the experience the
// conversation started from. That isn't decoration: enforce_explorer_chat lets a
// contextual opener through unconditionally, while a cold DM to someone you've never
// spoken to requires earned explorer status and is rejected server-side (42501).
export async function sendMessage({ senderId, recipientId, body, storyId, postId }) {
