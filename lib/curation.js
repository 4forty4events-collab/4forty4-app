...
// `curation` scopes the review queue: 'pending' (never curated — the default Inbox
// list), 'reviewed' (curated at least once), or 'all'. Pending is keyed on
// last_curated_at IS NULL so every fresh import surfaces automatically.
export async function fetchManageVenues(market, { curation = 'all' } = {}) {
  let q = supabase.from('venues').select('*').eq('market', market);
  if (curation === 'pending') q = q.is('last_curated_at', null);
  else if (curation === 'reviewed') q = q.not('last_curated_at', 'is', null);
  const { data, error } = await q.order('created_at', { ascending: false });
  if (error) throw error;
  return (data ?? []).map(normalizeVenue);
}

// Mark a venue reviewed (Skip — done without any edit) or reopen it to pending.
// Any normal Save already stamps it reviewed inside update_venue.
export async function setVenueCurated(venueId, curated) {
  const { error } = await supabase.rpc('set_venue_curated', { p_id: venueId, p_curated: curated });
  if (error) throw error;
}
