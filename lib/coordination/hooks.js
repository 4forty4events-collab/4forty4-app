...
import {
  ...
  fetchBlueprints, rateBlueprint, bumpBlueprintClone,
  setTripBudget, setTripStatus,
} from './coordinationRepository';
import { enrichTrips, enrichPlans } from './outingCards';
...
// ---- blueprints ------------------------------------------------------------
export function useBlueprints(market) {
  return useQuery({ queryKey: ['blueprints', market ?? null], queryFn: () => fetchBlueprints({ market }), staleTime: 30_000 });
}

export function useRateBlueprint() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ tripId, stars }) => rateBlueprint(tripId, stars),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['blueprints'] }),
  });
}

// Clone a blueprint into my own outings, then count the clone against the source.
export function useCloneBlueprint(userId) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (tripId) => {
      const trip = await cloneTrip(tripId);
      await bumpBlueprintClone(tripId);
      return trip;
    },
    onSuccess: () => {
...
// Batched card enrichment (covers / rosters / counts) for the Outings list. Keyed on
// the id sets so it only refetches when the visible outings change.
export function useOutingEnrichment(tripIds, planIds) {
  const tKey = Array.from(new Set(tripIds ?? [])).sort().join(',');
  const pKey = Array.from(new Set(planIds ?? [])).sort().join(',');
  const trips = useQuery({ queryKey: ['tripCards', tKey], queryFn: () => enrichTrips(tripIds), enabled: tKey.length > 0, staleTime: 30_000 });
  const plans = useQuery({ queryKey: ['planCards', pKey], queryFn: () => enrichPlans(planIds), enabled: pKey.length > 0, staleTime: 30_000 });
  return { tripsMap: trips.data ?? {}, plansMap: plans.data ?? {} };
}
...
export function useUpdateTripItem(tripId) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ itemId, dayDate, note, sortOrder, startTime }) => updateTripItem(itemId, { dayDate, note, sortOrder, startTime }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['tripItinerary', tripId] });
      qc.invalidateQueries({ queryKey: ['tripCards'] }); // a stop time can change the card's TONIGHT badge
    },
  });
}
...
// ---- trip meta (budget / cancelled) ----------------------------------------
// Owner-only setters behind SECURITY DEFINER RPCs. Both refresh the itinerary AND
// the Outings list, since each drives a card affordance (budget bar / CANCELLED).
function useTripMetaMutation(tripId, userId, fn) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: fn,
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['tripItinerary', tripId] });
      qc.invalidateQueries({ queryKey: ['trips', userId ?? null] });
      qc.invalidateQueries({ queryKey: ['tripCards'] });
    },
  });
}
export function useSetTripBudget(tripId, userId) {
  return useTripMetaMutation(tripId, userId, ({ budget, currency }) => setTripBudget(tripId, budget, currency));
}
export function useSetTripStatus(tripId, userId) {
  return useTripMetaMutation(tripId, userId, (status) => setTripStatus(tripId, status));
}
