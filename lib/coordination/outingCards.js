...
export async function enrichTrips(tripIds) {
  ...
  const [{ data: items }, { data: parts }] = await Promise.all([
    supabase.from('trip_items')
      .select('trip_id, sort_order, day_date, start_time, venue:venues(cover_image_url, price_per_person), event:events(cover_image_url, price_per_person)')
      .in('trip_id', ids)
  ...
  const out = {};
  ids.forEach((id) => { out[id] = { cover: null, placeCount: 0, spent: 0, earliestTime: null, memberCount: 0, members: [] }; });
  for (const it of items ?? []) {
    const o = out[it.trip_id]; if (!o) continue;
    o.placeCount += 1;
    if (!o.cover) o.cover = it.venue?.cover_image_url ?? it.event?.cover_image_url ?? null;
    const price = it.venue?.price_per_person ?? it.event?.price_per_person;
    if (price != null) o.spent += Number(price);
    if (it.start_time && (!o.earliestTime || it.start_time < o.earliestTime)) o.earliestTime = it.start_time;
  }
