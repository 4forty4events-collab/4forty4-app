...
export function normalizeTripItem(row) {
  ...
  return {
    ...
    startTime: row.start_time ?? null,        // "HH:MM" | null
    sortOrder: row.sort_order ?? 0,
    note: row.note ?? null,
    addedBy: row.added_by ?? null,
    title: formatVenueTitle(target?.name ?? target?.title ?? null),
    coverImageUrl: target?.cover_image_url ?? null,
    pricePerPerson: target?.price_per_person != null ? Number(target.price_per_person) : null,
    createdAt: row.created_at,
  };
}
...
export async function fetchTripItinerary(tripId) {
  const [{ data: trip, error: tErr }, { data: parts, error: pErr }, { data: items, error: iErr }] = await Promise.all([
    ...
    supabase.from('trip_items')
      .select('*, venue:venues(id, name, cover_image_url, price_per_person), event:events(id, title, cover_image_url, price_per_person)')
...
export async function addTripItem(tripId, target, { dayDate = null, note = null, sortOrder = 0, addedBy = null } = {}) {
  ...
  const { data, error } = await supabase.from('trip_items')
    .insert(row).select('*, venue:venues(id, name, cover_image_url, price_per_person), event:events(id, title, cover_image_url, price_per_person)').single();
  if (error) throw error;
...
export async function updateTripItem(itemId, { dayDate, note, sortOrder, startTime } = {}) {
  const patch = {};
  ...
  const { data, error } = await supabase.from('trip_items')
    ...
    .select('*, venue:venues(id, name, cover_image_url, price_per_person), event:events(id, title, cover_image_url, price_per_person)').single();
  if (error) throw error;
  return normalizeTripItem(data);
}
...
export async function addTripItems(tripId, items, addedBy = null) {
  ...
  const { data, error } = await supabase.from('trip_items')
    .insert(rows).select('*, venue:venues(id, name, cover_image_url, price_per_person), event:events(id, title, cover_image_url, price_per_person)');
  if (error) throw error;
...
export async function setTripBudget(tripId, budget, currency) {
  const { error } = await supabase.rpc('set_trip_budget', { p_trip: tripId, p_budget: budget, p_currency: currency ?? null });
  if (error) throw error;
}
...
// ---- blueprints (public, cloneable, rateable outings) ----------------------
export async function fetchBlueprints({ market, limit = 30 } = {}) {
  const { data, error } = await supabase.rpc('list_blueprints', { p_market: market ?? null, p_limit: limit });
  if (error) throw error;
  return (data ?? []).map((t) => ({
    id: t.id,
    title: t.title,
    ...
    cloneCount: Number(t.clone_count ?? 0),
...
export async function rateBlueprint(tripId, stars) {
  const { error } = await supabase.rpc('rate_blueprint', { p_trip: tripId, p_stars: stars });
  if (error) throw error;
}

// Best-effort clone counter — a failure here must never block the clone itself.
export async function bumpBlueprintClone(tripId) {
  await supabase.rpc('bump_blueprint_clone', { p_trip: tripId }).then(() => {}).catch(() => {});
}
