...
// Straight-line distance (m) from a {lat,lng} origin to a listing's coords, or null
// if either is missing. Used to bias the Outing Architect toward what's near you.
function haversineM(origin, listing) {
  if (origin?.lat == null || origin?.lng == null || listing?.latitude == null || listing?.longitude == null) return null;
  const R = 6371000, toRad = (d) => (d * Math.PI) / 180;
  const dLat = toRad(listing.latitude - origin.lat);
  const dLng = toRad(listing.longitude - origin.lng);
  const s = Math.sin(dLat / 2) ** 2 + Math.cos(toRad(origin.lat)) * Math.cos(toRad(listing.latitude)) * Math.sin(dLng / 2) ** 2;
  return 2 * R * Math.asin(Math.sqrt(s));
}

// OUTING ARCHITECT — the flagship. Composes a varied, budget-fit day out from real
// catalog venues WITHOUT persisting (so the UI can preview + reshuffle before the
// user commits). Same engine as auto-build (fetchCandidates + selectCombination);
// adds an optional near-me bias (soft 20km radius, falls back to market-wide when
// too thin). `near` = { lat, lng }. Attaches distanceM to each pick when known.
export async function composeOuting({ market, planType = 'single_day', budget, categories = null, near = null }, normalizeVenue, normalizeEvent) {
  const all = await fetchCandidates({ plan_type: planType, market }, { categories }, normalizeVenue, normalizeEvent);
  let pool = all.filter((l) => l.plannerCost <= budget);

  if (near) {
    const nearby = pool
      .map((c) => ({ c, d: haversineM(near, c) }))
      .filter((x) => x.d != null && x.d <= 20000)
      .map((x) => x.c);
    if (nearby.length >= 3) pool = nearby; // enough nearby to build from; else keep all
  }

  const { picked, spent, typeCount } = selectCombination(pool, budget, planType);
  const withDistance = picked.map((p) => ({ ...p, distanceM: near ? haversineM(near, p) : null }));
  return { picked: withDistance, spent, typeCount, candidateCount: pool.length };
}

// Persist a previewed outing as a budget_plan + its items (frozen costs + composed
// meals), reusing the same primitives as auto-build. Returns the created plan.
export async function saveComposedOuting(userId, { name, budget, currency, market, planType = 'single_day' }, picks) {
  const plan = await createPlan(userId, { name: name || null, totalBudget: budget, currency, market, planType });
