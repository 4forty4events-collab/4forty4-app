-- Filter EXPIRED events out of discovery/search: an event is "active" until its
-- end_time passes (coalesce to start_time when open-ended). This keeps in-progress
-- multi-hour/multi-day events visible instead of the old start_time >= now() which
-- dropped them the moment they began. Only the events WHERE clause changes.
