-- Thread the cover image through publish. The app uploads to R2 first and
-- passes the permanent public URL; null when no image was chosen.
-- Adding a param changes the signature, so drop the old overloads first to
-- avoid PostgREST "function is not unique" ambiguity.
drop function if exists public.publish_venue(uuid, text, text, text[], text, text, text);
drop function if exists public.publish_event(uuid, text, text, text[], text, text, date, text, numeric, text, text, text);
...
begin
  ...
  insert into public.venues (name, category, tags, description, address, market, cover_image_url)
  values (p_title, p_category, coalesce(p_tags, '{}'), p_description, p_address, p_market, p_cover_image_url)
