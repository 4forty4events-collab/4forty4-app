-- Reconcile venues/events with the content_drafts pipeline:
-- add market + tags (didn't exist), and generic price/currency on events
-- (price_dzd alone can't represent a USD/ZW price).
...
alter table public.events
  ...
  add column currency text check (currency in ('DZD','USD')),
  add column price_note text;

-- Publish a venue draft directly.
...
begin
  ...
  insert into public.venues (name, category, tags, description, address, market)
  values (p_title, p_category, coalesce(p_tags, '{}'), p_description, p_address, p_market)
...
-- Publish an event draft: match-or-create the venue by normalized name
-- (scoped to market, so DZ and ZW can't collide), then insert the event.
-- Done as one atomic function so a double-tap can't create duplicate venues.
