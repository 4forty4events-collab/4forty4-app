-- Images-first ranking: listings WITH a cover photo sort ABOVE those without, in
-- every category / shelf / search result, without touching keyset pagination.
--
-- The trick: fold a "has cover" bias straight into sort_v (still a single monotonic
-- key), so the cursor {v, id} and the repository stay unchanged. A photo-less item
-- is pushed to the very bottom of its result set regardless of sort direction:
--   * descending sorts (recent/rating/trending/relevance): +BIAS to items WITH a
--     cover -> they land at the top.
--   * ascending sort (distance): +BIAS to items WITHOUT a cover -> they sink last.
-- Condition simplifies to `v_asc <> has_cover`. BIAS (1e15) exceeds every mode's
-- sort_v spread yet stays well under 2^53, so integer equality (the tiebreak) is
-- still exact. "Has cover" = the same non-empty cover_image_url the cards render.
